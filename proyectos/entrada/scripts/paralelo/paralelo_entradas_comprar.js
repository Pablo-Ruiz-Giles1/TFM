module.exports = async callback => {
  try {
    ...
    console.log(`Direccion entrada: ${entradas.address}`);
    ...
    for (let i = 0; i < totalEntradas; i++) {
      comprasPromises.push(
        entradas.comprarEntrada(i, accounts[0]).then(tx => {
          return imprimirInfoTransaccion(tx.tx, `Comprada entrada numero:`, i);
        })
      );
    }
    
    const comprasTransacciones = await Promise.all(comprasPromises);
    imprimir("Posterior");
