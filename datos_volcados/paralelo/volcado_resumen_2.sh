...
filename="${archivo%.*}_resumen2.csv"
...
do
  # Si la línea es "--2--", se activa la bandera para empezar a procesar líneas
  if [[ $linea == "--2--" ]]
  then
    procesando=true
  # Si la línea es "--3--", se desactiva la bandera para dejar de procesar líneas
  elif [[ $linea == "--3--" ]]
  then
    procesando=false
